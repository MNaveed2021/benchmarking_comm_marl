...
from b_utils_algos_ncomm_rware import OnPolicyBuffer, MultiAgentOnPolicyBuffer, Scheduler
from c_policies_ncomm_rware import (LstmPolicy, FPPolicy, ConsensusPolicy, NCMultiAgentPolicy,
...
class IA2C:
    ...
    def _init_scheduler(self, model_config):
        ...
        #print("lr_init: ", self.lr_init)  # By me
        self.lr_decay = model_config.get('lr_decay')
        #print("lr_decay: ", self.lr_decay)  # By me
        if self.lr_decay == 'constant':
    ...
    def _init_train(self, model_config, distance_mask, coop_gamma):
        ...
        #print("v_coef: ", self.v_coef)  # By me
        self.e_coef = model_config.getfloat('entropy_coef')
        #print("entropy_coef: ", self.e_coef)  # By me
        self.max_grad_norm = model_config.getfloat('max_grad_norm')
        # init optimizer
        alpha = model_config.getfloat('rmsp_alpha')
        #print("rmsp_alpha: ", alpha)  # By me
        epsilon = model_config.getfloat('rmsp_epsilon')
        #print("rmsp_epsilon: ", epsilon)  # By me
        # Root Mean Squared Propagation (RMSprop) used here as optimizing algo/ optimizer
    ...
    # "self.n_step" given here to update "lr" for the batch size
    # optim.param_groups is a list of the different weight groups
    # which can have different learning rates.
    def _update_lr(self):
...
class MA2C_NC(IA2C):
    ...
    def add_transition(self, ob, p, action, reward, value, done):
        ...
        # np.clip() used to clip (limit) the values in the array
        if self.reward_clip > 0:
            reward = np.clip(reward, -self.reward_clip, self.reward_clip)
    ...
    def backward(self, Rends, dt, summary_writer=None, global_step=None):
        ...
        self.policy.backward(obs, ps, acts, dones, Rs, Advs, self.e_coef, self.v_coef,
                             summary_writer=summary_writer, global_step=global_step)
        # Gradient clipping during backward pass to fix exploding gradients
        if self.max_grad_norm > 0:
            nn.utils.clip_grad_norm_(self.policy.parameters(), self.max_grad_norm)
        self.optimizer.step()
        if self.lr_decay != 'constant':
